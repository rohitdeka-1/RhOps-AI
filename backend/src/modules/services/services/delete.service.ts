...
export class DeleteServiceService {
    ...
    async deleteService(clusterId: string, userId: string, name: string, namespace: string = 'default') {
        ...
        if (agentManager.isAgentConnected(clusterId)) {
            return await agentManager.executeTool(clusterId, 'delete_service', { name, namespace });
        }
        ...
        return await servicesClient.deleteService(name, namespace);
    }
}

...
export class DeleteNamespaceService {
    ...
    async deleteNamespace(clusterId: string, userId: string, name: string) {
        ...
        if (agentManager.isAgentConnected(clusterId)) {
            return await agentManager.executeTool(clusterId, 'delete_namespace', { name });
        }
        ...
        return await namespacesClient.deleteNamespace(name);
    }
}

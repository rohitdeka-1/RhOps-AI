...
function StorageNode({ data }: { data: any }) {
  const { label, engine, status = "Running" } = data; // engine = 'postgres' | 'redis' | 'mysql' | 'mongodb' | 'elasticsearch' | undefined
  ...
  return (
    <div className="relative group min-w-[220px] bg-[#1a1b1e] border border-[#2e3036] rounded-xl shadow-lg hover:border-primary/50 cursor-pointer overflow-hidden text-white flex flex-col">
      ...
      <div className="p-4 flex items-center gap-3">
        <div className="p-2 bg-[#2e3036] rounded-md text-blue-400">
